...
It runs discovery, crawling, HTML cleaning, hard filtering, thematic scoring, optional TF IDF inference, candidate selection, lead enrichment, and run metadata generation for one selected source registry.

Outputs are written to a run scoped directory and do not overwrite frozen evaluation artifacts.

Default lead generation uses `score_only` and the deterministic thematic score.
...
The recommended high recall operational mode uses the evaluated `score_or_tfidf` strategy.

It requires an explicit TF IDF model artifact.

```bash
PYTHONPATH=src python -m changescout.cli run \
  --config-dir config \
  --source-registry so_media \
  --canton-id so \
  --run-id run_hybrid_001 \
  --output-root artifacts/runs \
  --html-root data/crawling \
  --lead-threshold 0.10 \
...
Optional GeoAdmin enrichment can be enabled explicitly.
...
artifacts/runs/<run_id>/
  ...
  scored.jsonl
  scored_with_tfidf.jsonl                  # only when TF IDF inference is enabled
  leads.jsonl
  leads.csv
  leads_with_locations.jsonl
  leads_with_locations.csv
  leads_with_geoadmin_locations.jsonl      # only when GeoAdmin enrichment is enabled
  leads_with_geoadmin_locations.csv        # only when GeoAdmin enrichment is enabled
  monitoring_summary.json                  # generated by scripts/build_monitoring_summary.py
  monitoring_summary.md                    # generated by scripts/build_monitoring_summary.py
  reports/
    ...
    filter_report.json
    scoring_report.json
    tfidf_inference_report.json            # only when TF IDF inference is enabled
    lead_generation_report.json
    location_hinting_report.json
    geoadmin_location_hinting_report.json  # only when GeoAdmin enrichment is enabled
  metadata/
    run_metadata.json
  logs/
    run.log
...
Frozen annotation datasets, evaluation splits, method comparisons, hybrid evaluation reports, and LLM explainability evaluation outputs are not overwritten by operational runs.
...
This trains the operational TF IDF plus Logistic Regression actionable model artifact.

The positive actionable target is `confirmed_relevant` and `needs_review`.
...
Operational runs must load this artifact explicitly.

They must not retrain inside the monitoring run.

See `docs/model_artifacts.md`.

### Run standalone TF IDF inference
...
This adds `tfidf_actionable_probability` and related model metadata fields to scored records.

It is a learned review signal, not a confirmed TLM relevance decision.
...
8. optionally apply operational TF IDF inference
...
12. write run metadata, reports, and logs
...
Contains scored documents enriched with `tfidf_actionable_probability`.

This file is written only when a TF IDF model artifact is provided.
...
`artifacts/runs/<run_id>/leads_with_locations.jsonl`

Contains selected leads enriched with local deterministic location hints.

`artifacts/runs/<run_id>/leads_with_locations.csv`

CSV version of the locally enriched leads.

`artifacts/runs/<run_id>/leads_with_geoadmin_locations.jsonl`

Contains selected leads enriched with optional GeoAdmin Search API hints.

This file is written only when GeoAdmin enrichment is enabled.

`artifacts/runs/<run_id>/monitoring_summary.json`

Machine readable scoped monitoring summary.

`artifacts/runs/<run_id>/monitoring_summary.md`

Human readable scoped monitoring summary.
...
### Operational model artifact outputs

`data/models/tfidf_actionable/tfidf_actionable_v1/model.joblib`

Serialized operational TF IDF plus Logistic Regression model.

`data/models/tfidf_actionable/tfidf_actionable_v1/metadata.json`

Model metadata with target definition, training dataset hash, train and test split names, model parameters, threshold, and evaluation metrics.

`data/models/tfidf_actionable/tfidf_actionable_v1/test_predictions.csv`

Test split predictions generated during artifact training.

See `docs/model_artifacts.md`.

### Operational TF IDF inference outputs

`artifacts/runs/<run_id>/scored_with_tfidf.jsonl`

Scored operational records enriched with:
...
Contains record count, model version, probability summary, threshold, and predicted actionable count.

### Scoped monitoring summary outputs

`artifacts/runs/<run_id>/monitoring_summary.json`

Machine readable summary for one scoped operational run.

`artifacts/runs/<run_id>/monitoring_summary.md`

Human readable summary for one scoped operational run.

The summary reads scoped run metadata and scoped reports.

It does not read global reproduction artifacts unless explicitly configured.
...
9. Scoped lead enrichment with local location hints
10. Optional GeoAdmin lead enrichment
11. Scoped monitoring summary
12. Human review
13. MVP reproduction run
14. Evaluation dataset construction
15. Baseline evaluation
16. Local LLM evaluation
17. Task specific method comparison
18. Aligned method comparison
19. Hybrid lead selection evaluation
20. LLM explainability output evaluation
21. Evaluation report package construction
...
## Current lead generation and candidate selection behavior

Operational lead generation is now controlled by an explicit candidate selection mode.
...
`score_only` is the default mode.

It uses `thematic_score >= lead_threshold` and preserves the original deterministic baseline behavior.

`score_or_tfidf` is the recommended high recall operational mode.

It selects a record if either condition is true:

* `thematic_score >= lead_threshold`
* `tfidf_actionable_probability >= tfidf_threshold`

`score_or_tfidf` requires an explicit TF IDF model artifact and uses `scored_with_tfidf.jsonl` as input.

The selected lead records include:
...
The lead order is deterministic.

The sort key is based on selection score, thematic score, title, and URL.

The operational `score_or_tfidf` implementation was validated on a noisy Solothurn media registry.

In that validation run, the selector produced 29 leads from 323 scored records.

This confirms that the hybrid selector does not trivially select all records on broader source surfaces.

Zürich selected almost all records because the Zürich registry consists mainly of curated road project pages.

LLM predictions are not used as hard exclusion signals in operational candidate selection.

## Current geographic hinting behavior

* local location hinting runs only on selected leads
...
* scoped operational enrichment writes under `artifacts/runs/<run_id>/`
* standalone scripts are thin wrappers around reusable `src/changescout` functions
...
* provide scheduling, retries, alerting, or production operations handover
...
* the original MVP reproduction run still uses `thematic_score >= 0.10`; operational runs can now use `score_only` or `score_or_tfidf`
